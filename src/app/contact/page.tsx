export default function Contact() {
  return (
    <div className="page">
      <h1>Contact Info </h1>
      <h2>Dr. J. Srinibabu, Founder</h2>
      <h3>Phone: +91 75699 90821 / +91 9444532409</h3>
      <h3>
        Email: <a target="_blank" href="mailto:srinimusic@gmail.com">srinimusic@gmail.com</a>
      </h3>
      <h3>
        Youtube Channel:&nbsp;
        <a target="_blank" href="https://www.youtube.com/@srikalaranjanischoolofmusi9057">
          Sri Kalaranjani School of Music
        </a>
      </h3>

      <h3>
        <a target="_blank" href="https://docs.google.com/forms/d/e/1FAIpQLScUoTf033ZS5HNZKV3mOHrXojs1Ij4h86UJUltGE54r2GCU3A/viewform?embedded=true">
          Register Here For Shri Vishnu Sahasranamam Dasa Sahasra Galarchana
        </a>
      </h3>
      <h3>GPAY NO.90438 75147</h3>
      <br />
      <h2>Kavin Velmurugan, US Coordinator</h2>
      <h3>Phone: +1 (571) 283-5287</h3>
      <h3>
        Email: <a target="_blank" href="mailto:dev.velp@gmail.com">dev.velp@gmail.com</a>
      </h3>
      <br />
      <h2>Veda Srikanth, Canada Coordinator</h2>
      <h3>Phone: +1 (343) 576-3621</h3>
      <h3>
        Email: <a target="_blank" href="mailto:VedaSrikanthan@gmail.com">VedaSrikanthan@gmail.com</a>
      </h3>
      {/*<iframe src="https://docs.google.com/forms/d/e/1FAIpQLScUoTf033ZS5HNZKV3mOHrXojs1Ij4h86UJUltGE54r2GCU3A/viewform?embedded=true"></iframe>*/}
    </div>
  );
}
